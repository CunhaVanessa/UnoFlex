...
class ActorInterface():
    ...
    def createMenuDesign(self) -> None:
        ...
        game_images_directory = os.path.join(parent_directory, "src", "game_images")

        self.__background_img = PhotoImage(file = os.path.join(game_images_directory, f'BackgroundUnoFlex.png'))
        background = self.__canvas.create_image(0, 0,image=self.__background_img,anchor="nw")
        self.__button_menu = PhotoImage(file = os.path.join(game_images_directory, f'ButtonJogar.png'))
        button_start = self.__canvas.create_image(250, 600, image=self.__button_menu)
    ...
    def startMenu(self) -> None:
        ...
        player_name = simpledialog.askstring(title='Identificação do Jogador(a)', prompt= 'Qual seu nome?')
        self.__window.mainloop()


    def createTableDesign(self) -> None: 
        self.__background_img = PhotoImage(file = f"src/game_images/BackgroundMesa.png")
        background = self.__canvas.create_image(0, 0, image=self.__background_img,anchor="nw")

        self.__button_gritar_uno = PhotoImage(file = f"src/game_images/ButtonUno.png")
        button_start = self.__canvas.create_image(640, 80, image=self.__button_gritar_uno)
        self.__canvas.tag_bind(button_start, "<Button-1>", lambda x: print("Dizer UNO!"))


        self.__button_passar_turno = PhotoImage(file = f"src/game_images/ButtonPassarVez.png")
        button_passar_turno = self.__canvas.create_image(800, 300, image=self.__button_passar_turno)
        self.__canvas.tag_bind(button_passar_turno, "<Button-1>", lambda x: print("Passar Turno!"))
        

        self.__button_mover_mao_esq = PhotoImage(file = f"src/game_images/ButtonSetaEsquerda.png")
        button_mover_mao_esq = self.__canvas.create_image(340, 670, image=self.__button_mover_mao_esq)
        self.__canvas.tag_bind(button_mover_mao_esq, "<Button-1>", lambda x: print("Mover mão a esquerda"))


        self.__button_mover_mao_dir = PhotoImage(file = f"src/game_images/ButtonSetaDireita.png")
        button_mover_mao_dir = self.__canvas.create_image(940, 670, image=self.__button_mover_mao_dir)
        self.__canvas.tag_bind(button_mover_mao_dir, "<Button-1>", lambda x: print("Mover mão a direita"))
    ...
    def createCardButtons(self):
        ...
        card_width = 80  # Largura da carta (pode ser ajustada conforme suas imagens)
        ...
        individual_cards = [
            ...
            {"image": self.__dict_of_cards[f"card_0_90"], "x_position": 1175, "y_position": 450},
            ...
            {"image": self.__dict_of_cards[f"card_31_360"], "x_position": 750, "y_position": 550},
            {"image": self.__dict_of_cards[f"card_18_360"], "x_position": 850, "y_position": 550},
            {"image": self.__dict_of_cards[f"card_10_360"], "x_position": 950, "y_position": 550},
        ...
        for card_info in individual_cards:
            ...
            self.__canvas.tag_bind(button_card, "<Button-1>", lambda event, img=card_image: self.on_card_click(img))
    ...
    def loadCardImages(self) -> None:
            ...
            for i in range(104):
                ...
                image=Image.open(os.path.join(uno_cards_directory, f'card_{i}.png'))
                img=image.resize((80, 130))
                self.__dict_of_cards[f"card_{i}_360"] = ImageTk.PhotoImage(img.rotate(360, expand=True))
